...
class TextPageSource(menus.ListPageSource):
    ...
    async def format_page(self, menu: Any, content: Any) -> str:
        maximum = self.get_max_pages()
        if maximum > 1:
...
class SimplePageSource(menus.ListPageSource):
    async def format_page(self, menu: Any, entries: list[Any]) -> str:
        pages = []
...
class SimplePages(RoboPages):
    ...
    def __init__(self, entries: list[Any], *, ctx: Context, per_page: int = 12):
